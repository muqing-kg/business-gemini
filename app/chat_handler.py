...
def stream_chat_realtime_generator(jwt: str, sess_name: str, message: str, 
                                   ...
                                   host_url: str = None,
                                   show_thinking: bool = False, thinking_mode: str = "html") -> Generator[str, None, None]:
    """真正的流式处理：边接收边解析边转发
    ...
    注意：
        ...
        thinking_mode: 输出模式 - 'html'使用折叠显示, 'reasoning_content'使用独立字段
    """
            ...
            for reply in answer.get("replies", []):
                ...
                if text and thought:
                    ...
                    if show_thinking:
                        ...
                        # 如果使用 reasoning_content 模式，实时发送思考内容
                        if thinking_mode == "reasoning_content" and chat_id and created is not None and model_name:
                            thinking_chunk = {
                                "id": chat_id,
                                "object": "chat.completion.chunk",
                                "created": created,
                    ...
                    # 如果使用 HTML 折叠模式且有收集到的思考内容，在正文前输出
                    if show_thinking and thinking_mode == "html" and thinking_parts and chat_id and created is not None and model_name:
                        thinking_content = "\n\n".join(thinking_parts)
                        ...
                        thinking_html_chunk = {
                            ...
                            "created": created,
                            "model": model_name,
                            "choices": [{"index": 0, "delta": {"content": thinking_html}, "finish_reason": None}]
                        }
                        yield f"data: {json.dumps(thinking_html_chunk, ensure_ascii=False)}\n\n"
                        thinking_parts.clear()  # 清空已发送的思考内容
