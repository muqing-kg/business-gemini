...
async function loadConfig() {
    try {
        ...
        if (showThinkingEl) {
            showThinkingEl.checked = configData.show_thinking_process || false;
        }
        const thinkingModeEl = document.getElementById('thinkingOutputMode');
        if (thinkingModeEl) {
            thinkingModeEl.value = configData.thinking_output_mode || 'html';
        }
        ...
        document.getElementById('autoRefreshInterval').value = configData.auto_refresh_interval || 2;
...
function updateAutoRefreshStatus() {
    const enabled = document.getElementById('autoRefreshCookie').checked;
    const interval = parseInt(document.getElementById('autoRefreshInterval').value) || 2;
    ...
    if (enabled) {
        ...
        statusText.innerHTML = `✓ 自动刷新已启用，系统将每 ${interval} 小时检查一次过期 Cookie，使用临时邮箱自动刷新`;
...
async function saveAutoRefreshSettings() {
    ...
    if (interval < 1 || interval > 24) {
        showToast('刷新间隔必须在 1-24 小时之间', 'warning');
        return;
    }
    ...
    try {
        ...
        showToast(`刷新设置已保存（间隔: ${interval} 小时）`, 'success');
        updateAutoRefreshStatus();
    } catch (e) {
